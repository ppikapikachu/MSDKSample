...
//        KeyManager.getInstance().setValue(KeyTools.createKey(GimbalKey.KeyPitchControlMaxSpeed),35,null);
        ...
        Log.i(TAG,"onPTZControl 参数："+ctrlType+"==="+ptzType+"==="+speedParam);
        float pitch = 0, yaw = 0;
        ...
        if (ctrlType == 1) {
            switch (ptzType) {
                ...
                case PTZ_ZOOM_OUT://缩小
                    ...
                    //        变焦
                    KeyManager.getInstance().setValue(KeyTools.createCameraKey(CameraKey.KeyCameraZoomRatios,
                                    ComponentIndexType.LEFT_OR_MAIN, CameraLensType.CAMERA_LENS_ZOOM), v2,
                            new CommonCallbacks.CompletionCallback() {
                                @Override
                                public void onSuccess() {
                                    Log.i(TAG, "放缩成功");
            ...
            Log.i(TAG,"云台偏航角范围============="+Movement.getInstance().getGimbalYawRange());
...
//        PT值小于零，云台回中
        ...
        Log.i(TAG,"当前遥控屏幕的xy"+width+"==="+height);
...
//        zoomTargetPointInfo.setY((double) y);
        ...
        Log.i(TAG, x + "当前遥控的width*height屏幕x和y对应的值" + y);
...
//        }
        ...
        Log.i(TAG,Movement.getInstance().getGimbalPitchRange()+"===云台的角度范围==="+Movement.getInstance().getGimbalYawRange()
...
//            @Override
//            public void onSuccess(EmptyMsg emptyMsg) {
//                Log.i(TAG, x + ":" + y + "-----------------------指点对焦完成");
//            }
//
//            @Override
//            public void onFailure(@NonNull IDJIError idjiError) {
//                Log.i(TAG, "-----------------------指点对焦失败" + idjiError);
//            }
//        });
...
//           将所有的Callbacks和Messages全部清除掉,移除Handler中所有的消息和回调，避免内存泄露
    ...
    private void sendMobilePos(int num) {
        Jni28181AgentSDK.getInstance().sendMobilePos(this.subID, "Movement.getInstance().getDevieGBCode()",
                ...
                Double.parseDouble(Movement.getInstance().getYaw()), Movement.getInstance().getCurrentAltitude());
        if (num > 0) {
    ...
    public int getNumberThree(int[] intarray, int number) {
        int temp = Math.abs(number - intarray[0]);
        int result = intarray[0];
        int j = 0;
        for (int i = 0 ;i<intarray.length;i++){
            int abs = Math.abs(number - intarray[i]);
            if (abs <= temp) {
                temp = abs;
                result = intarray[i];
                j = i;
