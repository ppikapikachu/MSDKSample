...
import com.example.msdksample.utils.FrameUtil;
...
//    ICameraStreamManager.CameraFrameListener listener1 = new ICameraStreamManager.CameraFrameListener() {
//        @Override
...
//                uri = FileProvider.getUriForFile(GaoTest.this, GaoTest.this.getPackageName() + ".fileprovider", file);
//            } else {
//                uri = Uri.parse("file://" + file.toString());
//            }
//
//            if (continueSendVideo && frameData != null) {
//                Log.i(TAG, "h264格式");
//                int re = manager.sendVideoWithARInfoToLocal(System.currentTimeMillis(), FileUtil.getFrameType(frameData), frameData,
//                        Float.parseFloat(String.valueOf(Movement.getInstance().getCameraZoomRatios())),//double转float
//                        Float.parseFloat(Movement.getInstance().getGimbalPitch()), Float.parseFloat(Movement.getInstance().getGimbalYaw()),
//                        Movement.getInstance().getAngleH(), Movement.getInstance().getAngleV(),
//                        Float.parseFloat(Movement.getInstance().getCurrentLongitude()),
//                        Float.parseFloat(Movement.getInstance().getCurrentLatitude()),
//                        Movement.getInstance().getAltitude(), absolutePath + "/12.h264");
//                Log.i(TAG, "传输视频流与 AR 信息流，直接上传到指定路径 sendVideoWithARInfoToLocal:" +  FileUtil.getFrameType(frameData) + FrameUtil.INSTANCE.checkFrameType(frameData));
//            } else {
//                Log.i(TAG, "传输视频流与 AR 信息流，直接上传到指定路径 sendVideoWithARInfoToLocal:" + "数据为空");
//            }
//        }
